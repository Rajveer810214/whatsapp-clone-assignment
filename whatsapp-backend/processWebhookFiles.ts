...
import { processWebhookPayload } from './src/processPayload.js';
...
const processAllFiles = async (): Promise<void> => {
  ...
  try {
    if (!fs.existsSync(filesDir)) {
      console.error(`❌ Directory ${filesDir} does not exist. Please create it and add your JSON files.`);
      console.log('💡 To create the directory and add files:');
      console.log('   mkdir webhook_files');
      console.log('   # Then copy your JSON files to the webhook_files/ directory');
      process.exit(1);
    }

    const files = fs.readdirSync(filesDir).filter(file => file.endsWith('.json'));
    
    if (files.length === 0) {
      console.log('❌ No JSON files found in webhook_files directory');
      console.log('💡 Please add your webhook JSON files to the webhook_files/ directory');
      process.exit(1);
    }
    ...
    const otherFiles = files.filter(f => !f.includes('message') && !f.includes('status')).sort();
    ...
    for (const file of sortedFiles) {
      ...
      try {
        ...
        console.error(`❌ Error processing ${file}:`, fileError instanceof Error ? fileError.message : 'Unknown error');
        errorCount++;
      }
    }
    ...
    const totalConversations = (await Message.distinct('conversationId')).length;
    ...
    if (sampleMessages.length > 0) {
      ...
      sampleMessages.forEach((msg, index) => {
        console.log(`  ${index + 1}. [${msg.conversationId}] ${msg.userName}: ${msg.body.substring(0, 50)}${msg.body.length > 50 ? '...' : ''}`);
      });
    }
